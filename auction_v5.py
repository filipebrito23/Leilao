...
def submit_bid_v5(player_id, team_id, amount, years, username, user_id, role, forced_team_id=None):
    ...
    with engine.begin() as conn:
        ...
        player_state_exists = conn.execute(text("""
            SELECT COUNT(*)
            FROM player_state
            WHERE player_id = :player_id
        """), {
    ...
    log_event_v5(
        ...
        "player",
        player_id,
        f"amount={amount}; years={years}; renewal={is_renewal}"
    )
...
def get_players_with_state_v5(position=None):
    query = """
        SELECT
            ...
            ps.is_renewal
        FROM players p
        LEFT JOIN teams owner
            ON owner.team_id = p.owner_team_id
        LEFT JOIN player_state ps
            ON ps.player_id = p.player_id
        LEFT JOIN teams t
            ON t.team_id = ps.active_team_id
    """
...
def get_bid_history_v5(player_id):
    with engine.begin() as conn:
        return conn.execute(text("""
            ...
            LEFT JOIN users u
                ON u.user_id = b.created_by_user_id
