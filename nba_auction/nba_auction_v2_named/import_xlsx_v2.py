...
from auth_v2 import create_user_v2

POSITION_SHEETS = ["PG", "PG_SG", "SG", "SG_SF", "SF", "SF_PF", "PF", "PF_C", "C"]


def import_caps_v2(xlsx_path):
...
def import_players_v2(xlsx_path):
...
def seed_default_admin_v2():
    with engine.begin() as conn:
        exists = conn.execute(text("SELECT COUNT(*) FROM users WHERE username = 'admin' ")).scalar()
    if not exists:
        create_user_v2("admin", "admin123", "admin", None)


def seed_team_users_v2():
    with engine.begin() as conn:
    ...
    for team_id, team_name in teams:
        ...
        create_user_v2(username, "123456", "team", team_id)
...
if __name__ == "__main__":
    ...
    init_db_v2()
    import_caps_v2(xlsx_path)
    import_players_v2(xlsx_path)
    seed_default_admin_v2()
    seed_team_users_v2()
    print("Importação v2 concluída com sucesso.")
