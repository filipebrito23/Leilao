...
def log_event(username, action, entity_type, entity_id=None, details=None):
...
def submit_bid(player_id, team_id, amount, years, username, user_id):
    now = datetime.now(timezone.utc)
    ...
    log_event(username, "SUBMIT_BID", "player", player_id, f"amount={amount}; years={years}; renewal={is_renewal}")
    return True, "Proposta registrada com sucesso."


def get_players_with_state(position=None):
    query = """
    SELECT
        p.player_id,
        p.player_name,
...
def admin_add_player(player_name, position, owner_team_id, username):
    with engine.begin() as conn:
        conn.execute(text("""
            INSERT INTO players(player_name, position, owner_team_id, status)
            VALUES(:player_name, :position, :owner_team_id, 'OPEN')
        """), {"player_name": player_name, "position": position, "owner_team_id": owner_team_id})
    ...
    log_event(username, "ADD_PLAYER", "player", player_id, f"name={player_name}; position={position}")


def admin_delete_player(player_id, username):
    with engine.begin() as conn:
    ...
    log_event(username, "DELETE_PLAYER", "player", player_id, None)


def admin_reopen_player(player_id, username):
    with engine.begin() as conn:
    ...
    log_event(username, "REOPEN_PLAYER", "player", player_id, None)


def admin_close_player(player_id, username):
    ...
    log_event(username, "CLOSE_PLAYER", "player", player_id, None)


def admin_delete_bid(bid_id, username):
    with engine.begin() as conn:
        ...
        latest = conn.execute(text("""
            SELECT bid_id, team_id, amount, years, created_at, is_renewal
            FROM bids
            WHERE player_id = :player_id
        ...
        if latest:
            ...
            conn.execute(text("""
                ...
                WHERE player_id = :player_id
            """), {"player_id": player_id})
    log_event(username, "DELETE_BID", "bid", bid_id, None)
