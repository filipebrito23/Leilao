...
from auth_v3 import authenticate_user_v3, get_all_users_v3, create_user_v3
from auction_v3 import close_expired_bids_v3, submit_bid_v3, get_players_with_state_v3, get_bid_history_v3, get_team_rows_v3, get_audit_rows_v3, admin_add_player_v3, admin_delete_player_v3, admin_reopen_player_v3, admin_close_player_v3, admin_delete_bid_v3

POSITIONS = ["Todas", "PG", "PG_SG", "SG", "SG_SF", "SF", "SF_PF", "PF", "PF_C", "C"]

st.set_page_config(page_title="Leilão NBA Fantasy v3", layout="wide")
init_db_v3()
close_expired_bids_v3()

if "user_v3" not in st.session_state:
    st.session_state.user_v3 = None
...
def format_remaining_v3(expires_at):
    if not expires_at:
        return "-"
    try:
...
if not st.session_state.user_v3:
    st.title("Leilão NBA Fantasy v3")
    st.subheader("Login")
    with st.form("login_form_v3"):
        ...
        if submitted:
            user = authenticate_user_v3(username, password)
            if user:
                st.session_state.user_v3 = user
                st.rerun()
            else:
...
st.sidebar.button("Sair", on_click=logout_v3)

st_autorefresh(interval=20000, key="auction_refresh_v3")
...
with main_tab:
    ...
    players = pd.DataFrame(get_players_with_state_v3(position))
    if not players.empty:
        players["tempo_restante"] = players["expires_at"].apply(format_remaining_v3)
        ...
        history = pd.DataFrame(get_bid_history_v3(selected_player))
...
with bid_tab:
    players = pd.DataFrame(get_players_with_state_v3("Todas"))
    ...
    with st.form("bid_form_v3"):
        if open_players.empty:
            ...
            st.write(f"Tipo atual: {'Renovação' if selected_row['is_renewal'] == 1 else 'Oferta'}")
            if user["role"] == "admin":
            ...
            if submitted:
                ok, msg = submit_bid_v3(player_id, team_id, amount, years, user["username"], user["user_id"], user["role"], user.get("team_id"))
                if ok:
                    st.success(msg)
                    st.rerun()
                else:
...
with cap_tab:
    cap_df = pd.DataFrame(get_team_rows_v3())
...
with admin_tab:
    if user["role"] != "admin":
        ...
        with admin_sub1:
            players_df = pd.DataFrame(get_players_with_state_v3("Todas"))
            st.dataframe(players_df, use_container_width=True, hide_index=True)
            with st.form("add_player_form_v3"):
                ...
                if add_submit and player_name:
                    admin_add_player_v3(player_name, position, owner_options[owner_name], user["username"])
                    st.success("Jogador adicionado.")
                    st.rerun()
            if not players_df.empty:
                target_player = st.selectbox("Selecionar jogador", players_df["player_id"].tolist(), format_func=lambda pid: players_df.loc[players_df["player_id"] == pid, "player_name"].iloc[0], key="admin_player_select_v3")
                c1, c2, c3 = st.columns(3)
                if c1.button("Fechar jogador"):
                    admin_close_player_v3(target_player, user["username"])
                    st.rerun()
                if c2.button("Reabrir jogador"):
                    admin_reopen_player_v3(target_player, user["username"])
                    st.rerun()
                if c3.button("Excluir jogador"):
                    admin_delete_player_v3(target_player, user["username"])
                    st.rerun()
        with admin_sub2:
            users_df = pd.DataFrame(get_all_users_v3())
            st.dataframe(users_df, use_container_width=True, hide_index=True)
            with st.form("create_user_form_v3"):
                ...
                if create_submit and new_username and new_password:
                    ...
                    create_user_v3(new_username, new_password, new_role, team_id_new)
                    st.success("Usuário criado.")
                    st.rerun()
        with admin_sub3:
            players_df = pd.DataFrame(get_players_with_state_v3("Todas"))
            if not players_df.empty:
                target_player_bids = st.selectbox("Jogador para gerenciar lances", players_df["player_id"].tolist(), format_func=lambda pid: players_df.loc[players_df["player_id"] == pid, "player_name"].iloc[0], key="bid_player_select_v3")
                bids_df = pd.DataFrame(get_bid_history_v3(target_player_bids))
                ...
                if not bids_df.empty:
                    ...
                    if st.button("Excluir lance selecionado"):
                        admin_delete_bid_v3(bid_id, user["username"])
                        st.success("Lance excluído.")
                        st.rerun()
        with admin_sub4:
            audit_df = pd.DataFrame(get_audit_rows_v3())
