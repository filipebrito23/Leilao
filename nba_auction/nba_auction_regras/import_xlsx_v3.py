...
from auth_v3 import create_user_v3

POSITION_SHEETS = ["PG", "PG_SG", "SG", "SG_SF", "SF", "SF_PF", "PF", "PF_C", "C"]


def import_caps_v3(xlsx_path):
...
def import_players_v3(xlsx_path):
...
def seed_default_admin_v3():
    with engine.begin() as conn:
        exists = conn.execute(text("SELECT COUNT(*) FROM users WHERE username = 'admin' ")).scalar()
    if not exists:
        create_user_v3("admin", "admin123", "admin", None)


def seed_team_users_v3():
    with engine.begin() as conn:
    ...
    for team_id, team_name in teams:
        ...
        create_user_v3(username, "123456", "team", team_id)
...
if __name__ == "__main__":
    ...
    init_db_v3()
    import_caps_v3(xlsx_path)
    import_players_v3(xlsx_path)
    seed_default_admin_v3()
    seed_team_users_v3()
    print("Importação v3 concluída com sucesso.")
