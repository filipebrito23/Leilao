...
from db_v3 import engine


def log_event_v3(username, action, entity_type, entity_id=None, details=None):
...
def get_team_cap_status_v3(conn, team_id):
...
def submit_bid_v3(player_id, team_id, amount, years, username, user_id, role, forced_team_id=None):
    ...
    with engine.begin() as conn:
        ...
        _, _, available = get_team_cap_status_v3(conn, team_id)
    ...
    log_event_v3(username, "SUBMIT_BID", "player", player_id, f"amount={amount}; years={years}; renewal={is_renewal}")
    return True, "Proposta registrada com sucesso."


def get_players_with_state_v3(position=None):
    query = """
...
def get_bid_history_v3(player_id):
    with engine.begin() as conn:
...
def get_audit_rows_v3(limit=200):
    with engine.begin() as conn:
...
def admin_add_player_v3(player_name, position, owner_team_id, username):
    with engine.begin() as conn:
    ...
    log_event_v3(username, "ADD_PLAYER", "player", player_id, f"name={player_name}; position={position}")


def admin_delete_player_v3(player_id, username):
    with engine.begin() as conn:
    ...
    log_event_v3(username, "DELETE_PLAYER", "player", player_id, None)


def admin_reopen_player_v3(player_id, username):
    with engine.begin() as conn:
    ...
    log_event_v3(username, "REOPEN_PLAYER", "player", player_id, None)


def admin_close_player_v3(player_id, username):
    ...
    log_event_v3(username, "CLOSE_PLAYER", "player", player_id, None)


def admin_delete_bid_v3(bid_id, username):
    with engine.begin() as conn:
        ...
        if latest:
            ...
            conn.execute(text("""
                ...
                WHERE player_id = :player_id
            """), {"player_id": player_id})
    log_event_v3(username, "DELETE_BID", "bid", bid_id, None)
