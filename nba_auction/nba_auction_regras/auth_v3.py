...
def create_user_v3(username, password, role, team_id=None):
    with engine.begin() as conn:
        ...
        """), {
            "username": username,
            "password_hash": hash_password_v3(password),
            "role": role,
            "team_id": team_id
        })


def authenticate_user_v3(username, password):
    with engine.begin() as conn:
    ...
    if not user or not user["is_active"]:
        return None
    if user["password_hash"] != hash_password_v3(password):
        return None
    return dict(user)


def get_all_users_v3():
    with engine.begin() as conn:
