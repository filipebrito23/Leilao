...
from db_v4 import engine


def log_event_v4(username, action, entity_type, entity_id=None, details=None):
...
def get_team_cap_status_v4(conn, team_id):
...
def close_expired_bids_v4():
    now = datetime.now(timezone.utc)
    with engine.begin() as conn:
        rows = conn.execute(text("SELECT player_id FROM player_state WHERE status = 'OPEN' AND expires_at IS NOT NULL AND expires_at < :now"), {"now": now.isoformat()}).fetchall()
        for row in rows:
...
def submit_bid_v4(player_id, team_id, amount, years, username, user_id, role, forced_team_id=None):
    ...
    with engine.begin() as conn:
        player = conn.execute(text("SELECT p.player_id, p.owner_team_id, p.status FROM players p WHERE p.player_id = :player_id"), {"player_id": player_id}).mappings().first()
        if not player:
        ...
        _, _, available = get_team_cap_status_v4(conn, team_id)
        current_active = conn.execute(text("SELECT active_bid_id, active_team_id, active_amount FROM player_state WHERE player_id = :player_id"), {"player_id": player_id}).mappings().first()
        released_amount = 0
        ...
        """), {"player_id": player_id, "team_id": team_id, "amount": amount, "years": years, "created_at": now.isoformat(), "is_renewal": is_renewal, "created_by_user_id": user_id})
        ...
        """), {"player_id": player_id, "bid_id": bid_id, "team_id": team_id, "amount": amount, "years": years, "active_at": now.isoformat(), "expires_at": expires_at.isoformat(), "is_renewal": is_renewal})
    log_event_v4(username, "SUBMIT_BID", "player", player_id, f"amount={amount}; years={years}; renewal={is_renewal}")
    return True, "Proposta registrada com sucesso."


def get_players_with_state_v4(position=None):
    query = """
    SELECT p.player_id, p.player_name, p.position, owner.team_name AS dono, t.team_name AS time_ativo,
           ps.active_amount AS proposta_ativa, ps.active_years AS anos, ps.active_at, ps.expires_at,
           ps.status, ps.is_renewal
    FROM players p
...
def get_bid_history_v4(player_id):
    with engine.begin() as conn:
        return conn.execute(text("""
            SELECT b.bid_id, t.team_name, b.amount, b.years, b.created_at, b.is_active, b.is_renewal, u.email AS created_by
            FROM bids b JOIN teams t ON t.team_id = b.team_id
            LEFT JOIN users u ON u.user_id = b.created_by_user_id
            WHERE b.player_id = :player_id ORDER BY b.created_at DESC
        """), {"player_id": player_id}).mappings().all()


def get_team_rows_v4():
    with engine.begin() as conn:
        return conn.execute(text("""
            ...
            FROM teams t LEFT JOIN bids b ON b.team_id = t.team_id
            GROUP BY t.team_id, t.team_name, t.cap_limit
            ORDER BY available_cap DESC
        """)).mappings().all()


def get_audit_rows_v4(limit=200):
    with engine.begin() as conn:
...
def admin_add_player_v4(player_name, position, owner_team_id, username):
    with engine.begin() as conn:
    ...
    log_event_v4(username, "ADD_PLAYER", "player", player_id, f"name={player_name}; position={position}")


def admin_delete_player_v4(player_id, username):
    with engine.begin() as conn:
    ...
    log_event_v4(username, "DELETE_PLAYER", "player", player_id, None)


def admin_reopen_player_v4(player_id, username):
    with engine.begin() as conn:
    ...
    log_event_v4(username, "REOPEN_PLAYER", "player", player_id, None)


def admin_close_player_v4(player_id, username):
    ...
    log_event_v4(username, "CLOSE_PLAYER", "player", player_id, None)


def admin_delete_bid_v4(bid_id, username):
    with engine.begin() as conn:
        ...
        if latest:
            ...
            """), {"bid_id": latest["bid_id"], "team_id": latest["team_id"], "amount": latest["amount"], "years": latest["years"], "active_at": latest["created_at"], "expires_at": (datetime.fromisoformat(latest["created_at"]) + timedelta(hours=24)).isoformat(), "player_id": player_id, "is_renewal": latest["is_renewal"]})
        else:
            conn.execute(text("UPDATE player_state SET active_bid_id = NULL, active_team_id = NULL, active_amount = NULL, active_years = NULL, active_at = NULL, expires_at = NULL, status = 'OPEN', is_renewal = 0 WHERE player_id = :player_id"), {"player_id": player_id})
    log_event_v4(username, "DELETE_BID", "bid", bid_id, None)
